...
class MapEnv(object):
    ...
    """ marl int addition """
    def get_action_set(self, agents, obs, method):
        # for a given observation, get the actions of the agents. See README.
        # Returns actions for the environment, actions for the agents. 
        env_actions, int_actions = dict(), list()
        for (idx, key) in enumerate(obs):
            obs_i = obs[key] 

            if method == 'train':
                action_i = agents[idx].action_callback(obs_i)
            elif method == 'evaluate':
                action_i = agents[idx].policy_callback(obs_i)

            env_actions[key] = action_i 
            int_actions.append(action_i) 
        
        return env_actions, int_actions 